...
new_top_level_prompt = """
    ...
    - Verified purchase history (including order ID, total amount, and shipping method)
    - Refund eligibility status

    Your task is to:
    1. Use the provided information to determine whether the refund should be processed.
    2. If eligible, call the refund processing tool or sub-agent to issue the refund.
    3. If not eligible, politely inform the user that the refund cannot be processed.
    4. End the conversation with a thank-you message and a few cute emojis like 🦀 or 🍬.
